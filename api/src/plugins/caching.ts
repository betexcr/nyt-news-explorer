...
async function cachingPlugin(fastify: FastifyInstance) {
  ...
  const getCacheControl = (request: FastifyRequest): string => {
    const url = request.url
  ...
  fastify.addHook('onSend', async (request, reply, payload) => {
    ...
    // Generate ETag
    const etag = generateETag(payload as string | Buffer)
  ...
  fastify.decorate('invalidateCache', async (patterns: string[]) => {
  ...
  fastify.decorate('warmupCache', async (endpoints: string[]) => {
